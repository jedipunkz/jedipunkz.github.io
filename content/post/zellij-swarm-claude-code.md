...
2026年2月に Claude Code Agent Teams という機能が出てきたのですが、あくまでも一つのタスクの中で複数のサブエージェントを稼働させるものだったので自分としては期待していたものと若干異なりました。
...
各エージェントは独立した git worktree 上で作業し、完了後に親エージェントが各ブランチをマージして後片付けまで行います。
...
```
swarm で実装して。チーム A は src/api/ のリファクタリング、チーム B は tests/ のテスト追加をして。
```
...
- **タスクの独立性が必須**: 複数エージェントが同じファイルを編集するとコンフリクトが発生します。タスクを設計する際はファイルの重複がないように意識する必要があります
- **Zellij 必須**: `$ZELLIJ` 環境変数が存在しない環境では動作しません
- **エージェント間の通信なし**: 各エージェントは独立して動くため、相互に状態を共有したり依存したりするタスクには対応していません
- **監視はポーリング**: イベント駆動ではなく 30 秒間隔のポーリングで監視しています
