---
title: "GCP Cloud Run + CI/CD パイプラインを Terraform でベストプラクティスに沿って構築する"
...
GCP 上でコンテナアプリケーションをサーバーレスに動かす Cloud Run は、運用コストの低さと自動スケーリングの手軽さから採用事例が増えています。ただ「Cloud Run を使うだけ」であれば比較的簡単ですが、本番環境として成立させるには Load Balancer、WAF、CI/CD パイプライン、IAM 設計、ネットワーク設計など多くのコンポーネントを組み合わせる必要があります。
...
今回紹介した構成は、Cloud Run を本番で使う際に必要になる要素をひと通り揃えたものになっています。特に意識したベストプラクティスをまとめると次のとおりです。
