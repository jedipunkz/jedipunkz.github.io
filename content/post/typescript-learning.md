...
- any 型 - すべての型を受け入れます。型チェックを無効化するため既存の JavaScript コードを移行する際などに使用しますが型安全性が失われるため多用は避けるべきです。
- unknown 型 - any と同様にすべての型を受け入れますが使用前に型チェックが必要です。型安全な any として推奨されます。
...
- [TypeScript 公式リファレンス](https://www.typescriptlang.org/docs/handbook/utility-types.html)
